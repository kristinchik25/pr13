...
## Краткий анализ
Самый эффективный запрос — первый, с JOIN , время выполнения 187.896 мс . Он использует индексы напрямую и не требует сортировки или агрегации, что делает его очень быстрым и оптимальным для больших данных.

Второй (GROUP BY) и третий (подзапрос WHERE IN) работают чуть дольше и требуют больше ресурсов на обработку.
